...
grid.arrange(arrangeGrob(arrangeGrob(arrangeGrob(PCA_plot, left=textGrob("A", vjust=-13, hjust=2, gp=gpar(fontface=2, fontsize=20))), arrangeGrob(tSNE_plot("none"), left=textGrob("B", vjust=-13, hjust=2, gp=gpar(fontface=2, fontsize=20))), ncol=2), PCA_tSNE_legend, nrow=2, heights=c(6,1)))
```
...
**Fig 5.** _Figure reports hierarchical clustering based on the 500 most variable genes. Fig 9A reports the clustering using a dendrogram while Fig 9B reports the same clustering using a circular packing plot._
...
venn_plot_2w_3w <- invisible(draw.pairwise.venn(sum(venn_data_logical[,6]), 
               ...
               cat.fontface=2,
               cat.just=list(c(0.5, -10), c(0.5, -10)),
               cat.default.pos = "text", 
               margin=c(0.3)))
...
**Fig 11.** _Figure reports the number of DE genes within and between contrasts Naive[2w]vsSCI[2w] and Naive[3w]vsSCI[3w]._
