...
PCA_plot <- ggplot(DT_principal_comp, aes(PC1, PC2))+
  ...
  annotate("text", x=DT_principal_comp[group=="SCI:2", mean(PC1)]+20, y=DT_principal_comp[group=="SCI:2", mean(PC2)], label="SCI&NPC[2w]", size=4, fontface=2)+
  annotate("text", x=DT_principal_comp[group=="Naive:2", mean(PC1)], y=DT_principal_comp[group=="Naive:2", mean(PC2)]-15, label ="Naive&NPC[2w]", size=4, fontface=2)+
  annotate("text", x=DT_principal_comp[group=="SCI:3", mean(PC1)], y=DT_principal_comp[group=="SCI:3", mean(PC2)]-12, label="SCI&NPC[3w]", size=4, fontface=2)+
  annotate("text", x=DT_principal_comp[group=="Naive:3", mean(PC1)], y=DT_principal_comp[group=="Naive:3", mean(PC2)]+15, label="Naive&NPC[3w]", size=4, fontface=2)+
...
dimension_plot_function <- function(data_set, dim_reduction_type, legend.position="none", textboolean=TRUE, cluster_type="none") {
  ...
  dim_plot <- ggplot(data_set, aes(V1, V2))+
    ...
    if(cluster_type!="none"){dim_plot <- dim_plot + 
      ...
      scale_color_manual(values=color_palette, labels=c("C1", "C2", "C3", "C4", "C5"))}
