...
* Samples 101-108 (1w SCI&Naive) + 111 (2w SCI) have double peaks in density of log-cpm plots. 101-108 had low alignment rates and 111 is a definite outlier in the MDS. Samples 101-108 and 111 are therefore removed.

***

# Project Description

**Hypothesis:** _The transcriptome of subventricular zone derived neural progenitor cells (NPCs) transplanted into a spinal cord injury environment is altered as compared to NPCs transplanted into uninjured spinal cord._
...
#10. Adding normalisation factor (=1 initially)
DT_meta_data <- DT_meta_data[,norm.factor:=1]
#11. Defining factor variables
...
#9. Bootsrapping proportion of variance for each principal component   
...
PCA_plot <- ggplot(DT_principal_comp, aes(PC1, PC2))+
  ...
  annotate("text", x=DT_principal_comp[group=="SCI:2W", mean(PC1)]+25, y=DT_principal_comp[group=="SCI:2W", mean(PC2)], label="SCI&NPC[2w]", size=4, fontface=2)+
...
```{r echo=F, warning=F, message=F, error=F, fig.height=6, fig.width=12}
#1. Plotting 2D dimension reduction graphs (PCA, tSNE)
PCA_tSNE_legend <- get_legend(tSNE_plot("bottom"))

grid.arrange(arrangeGrob(arrangeGrob(arrangeGrob(PCA_plot, left=textGrob("A", vjust=-10, hjust=-1, gp=gpar(fontface=2, fontsize=20))), arrangeGrob(tSNE_plot("none"), left=textGrob("B", vjust=-10, hjust=-1, gp=gpar(fontface=2, fontsize=20))), ncol=2), PCA_tSNE_legend, nrow=2, heights=c(6,1)))

```

**Fig 4.** _Figure reports the samples in low dimensional space following dimensionality reduction with PCA and t-SNE for the 500 genes with the highest variance. Ellipses are added to the samples for easier recognition of the study groups._
...
#2. Creating a dissimilarity structure for the 500 genes with highest variance
setkey(DT_lcpm, ENSEMBL_ID)
...
**Fig 5.** _Figure reports hierarchical clustering based on the 500 most variable genes. Fig 5A reports the clustering using a dendrogram while Fig 5B reports the same clustering using a circular packing plot._
...
cluster_legend <- get_legend(dimension_plot_function(PCA_cluster_data, "", cluster_type = "KM", legend.position = "bottom"))

#K-Means Clustering
grid.arrange(arrangeGrob(arrangeGrob(dimension_plot_function(PCA_cluster_data, "K-Means Clustering (3Cs)\n[PCA]", cluster_type = "KM"), left=textGrob("A", vjust=-9, hjust=-3, gp=gpar(fontface=2, fontsize=20))), arrangeGrob(dimension_plot_function(tSNE_cluster_data, "K-Means Clustering (3Cs)\n[tSNE]", cluster_type = "KM"), left=textGrob("B", vjust=-9, hjust=-3, gp=gpar(fontface=2, fontsize=20))), ncol=2), cluster_legend, nrow=2, heights=c(5,1))
#Affinity Propagation
grid.arrange(arrangeGrob(arrangeGrob(dimension_plot_function(PCA_cluster_data, "Affinity Propagation\n[PCA]", cluster_type = "AP"), left=textGrob("C", vjust=-9, hjust=-3, gp=gpar(fontface=2, fontsize=20))), arrangeGrob(dimension_plot_function(tSNE_cluster_data, "Affinity Propagation\n[tSNE]", cluster_type = "AP"), left=textGrob("D", vjust=-9, hjust=-3, gp=gpar(fontface=2, fontsize=20))), ncol=2), cluster_legend, nrow=2, heights=c(5,1))

```

**Fig 6.** _Figure reports result of K-means clsutering and affinity propagation algorithms implemented on samples following dimensionality reduction using PCA and t-SNE. In the case train-test split was required a 50:50 ratio (9:9 samples) was used instead of a traditional 80:20 ratio due to low number of samples in the test set._
...
**Fig 8.** _Figure reports the mean-variance relationship pre -and post application of the voom function. Fig 8A reports the average log-CPM against the quarter root of the variance. Fig 8B reports average log-CPM against the $log_2(st.dev)$. Blue line reports the average $log_2(st.dev)$. The red line is a linear trend fitted to the black dots. Each black dot represents a gene. Fig 8A illustrates that the variance is decreasing when the average expression is increasing. In Fig 8B the dependency is removed and the mean variance is unchanged when the average expression increases._      
...
**Fig 9.** _Figure reports hierarchical clustering of samples based on the 1000 genes with highest F-values. Fig 9A reports a dendrogram while Fig 9B reports a circular packing plot._ 
...
venn_plot_2w_3w <- invisible(draw.pairwise.venn(sum(venn_data_logical[,6]), 
               ...
               cat.fontface=2,
               cat.just=list(c(0.8, -10), c(0.2, -10)),
               cat.default.pos = "text", 
               margin=c(0.3)))
...
###SECTION OUTPUT
grid.arrange(arrangeGrob(gTree(children=venn_plot_2w), left=textGrob("A", gp=gpar(fontface="bold", fontsize=20), vjust=-13)), arrangeGrob(gTree(children=venn_plot_3w), left=textGrob("B", gp=gpar(fontface="bold", fontsize=20), vjust=-13)), ncol=2)
```

**Fig 10.** _Figure 10A reports the number of DE genes between contrasts Naive[2w] vs invitro, SCI[2w] vs invitro, Naive[2w] vs SCI[2w]. Figure 10B reports the number of DE genes between contrasts Naive[3w] vs invitro, SCI[3w] vs invitro and Naive[3w] vs SCI[3w]._
...
**Fig 11.** _Figure reports the number of DE genes within and between contrasts Naive[2w] vs SCI[2w] and Naive[3w] vs SCI[3w]._
...
```{r echo=F, warning=F, message=F, error=F}
#1. Mean-difference plot function
...
volcano_plot_function <- function(data_set){
  ...
  annotate("text", x=4, y=50, label="UP-REGULATED", fontface=2, alpha=0.6, size=5)+
  annotate("text", x=-4, y=50, label="DOWN-REGULATED", fontface=2, alpha=0.6, size=5)+
...
plot_grid(arrangeGrob(mean_difference_plot_function(DT_mean_difference_Naive), left=textGrob("E", vjust=-11, hjust=-2, gp=gpar(fontsize=20, fontface=2))), arrangeGrob(volcano_plot_function(DT_volcano_Naive), left=textGrob("F", vjust=-11, hjust=-2, gp=gpar(fontsize=20, fontface=2))))
...
```{r echo=F, warning=F, message=F, error=F}
kable(top_20_dge_2w, align=rep(c("l", "c", "c"), 2), col.names = rep(c("Gene", "log2(fold change)", "P-value (adjusted)"), 2))
```

**4B. Contrast: Naive[3w] vs SCI[3w]**  
```{r echo=F, warning=F, message=F, error=F}
kable(top_20_dge_3w, align=rep(c("l", "c", "c"), 2), col.names = rep(c("Gene", "log2(fold change)", "P-value (adjusted)"), 2))
```

**4C. Contrast: Naive[2w] vs Naive[3w]**  
```{r echo=F, warning=F, message=F, error=F}
kable(top_20_dge_Naive, align=rep(c("l", "c", "c"), 2), col.names = rep(c("Gene", "log2(fold change)", "P-value (adjusted)"), 2))
```

**4D. Contrast: SCI[2w] vs SCI[3w]**  
```{r echo=F, warning=F, message=F, error=F}
kable(top_20_dge_SCI, align=rep(c("l", "c", "c"), 2), col.names = rep(c("Gene", "log2(fold change)", "P-value (adjusted)"), 2))
```
...
#3. GO enrichment analysis function
GO_retreiver <- function(contrast){
  GO_out <- goana(DT_efit, coef=contrast, geneid = entrez_ID[,as.character(ENTREZID)], FDR=0.05, species="Rn") 
  GO_out <- data.table(topGO(GO_out, sort="down", n=10)) 
  return(GO_out)}

#4. KEGG pathway enrichment analysis function
KEGG_retriever <- function(contrast){
  KEGG_out <- kegga(DT_efit, coef=contrast, geneid = entrez_ID[,as.character(ENTREZID)], FDR=0.05, species="Rn")
  KEGG_out <- data.table(topKEGG(KEGG_out, sort="down", n=10))
  return(KEGG_out)}
```
...
```{r echo=F, warning=F, message=F, error=F}
kable(GO_retreiver(6), align=c("l", rep("c", 6)))
kable(KEGG_retriever(6), align=c("l", rep("c", 5)))
```

**5B. Contrast: Naive[3w] vs SCI[3w]**  

```{r echo=F, warning=F, message=F, error=F}
kable(GO_retreiver(9), align=c("l", rep("c", 6)))
kable(KEGG_retriever(9), align=c("l", rep("c", 5)))
```

**5C. Contrast: Naive[2w] vs Naive[3w]**  

```{r echo=F, warning=F, message=F, error=F}
kable(GO_retreiver(5), align=c("l", rep("c", 6)))
kable(KEGG_retriever(5), align=c("l", rep("c", 5)))
```

**5D. Contrast: SCI[2w] vs SCI[3w]**  

```{r echo=F, warning=F, message=F, error=F}
kable(GO_retreiver(10), align=c("l", rep("c", 6)))
kable(KEGG_retriever(10), align=c("l", rep("c", 5)))
```
...
```{r echo=F, warning=F, message=F, error=F}
#1. Creating heatmap data - function 
heatmap_data_function <- function(contrast, p_value_cutoff){
  data_out <- DT_logFC <- data.table(toptable(DT_efit, coef=contrast, number=Inf, genelist=DT_read_count_filtered[, ENSEMBL_ID]))
  data_out <- merge(data_out, DT_read_count[,.(ENSEMBL_ID, SYMBOL)], by.x="ID", by.y="ENSEMBL_ID")  
  data_out <- data_out[adj.P.Val<p_value_cutoff]
  setcolorder(data_out, c(1, 7, 2:6))
  data_out[,"SYMBOL"] <- data_out[,.(ifelse(is.na(SYMBOL), ID, SYMBOL))]
  
  data_out <- DT_lcpm[DT_lcpm[,ENSEMBL_ID] %in% data_out[,ID]]

  return(data_out)}

#2. Heatmap-function
heatmap_plot_function <- function(heatmap.data){
    heatmap.data <- data.matrix(heatmap.data[, 3:length(heatmap.data)])
...
heatmap_plot_function(heatmap_data_function(6, 1e-6))
```

**13B. Contrast: Naive[3w] vs SCI[3w] (FDR<1e-6)**

```{r echo=F, warning=F, message=F, error=F, fig.width=10, fig.height=10}
heatmap_plot_function(heatmap_data_function(9, 1e-6))
```

**13C. Contrast: Naive[2w] vs Naive[3w] (FDR<0.05)**

```{r echo=F, warning=F, message=F, error=F, fig.width=10, fig.height=10}
heatmap_plot_function(heatmap_data_function(5, 0.05))
```

**13D. Contrast: SCI[2w] vs SCI[3w] (FDR<1e-6)**

```{r echo=F, warning=F, message=F, error=F, fig.width=10, fig.height=10}
heatmap_plot_function(heatmap_data_function(10, 1e-6))
```

**Fig 13.** _Figure reports a heatmap with hierarchical clustering (indicated with dendrograms) using log-CPM values. Only significantly differentially expressed genes are included (genes with NA symbols were removed)._ 
